...
function UserPhotos() {
  ...
  return (
    <Box>
    {(!photos || photos.length === 0) && <Typography variant="h6" sx={{ p: 2 }}>Đang tải hoặc không có ảnh.</Typography>;}
      {photos.map((photo) => (
        <Card key={photo._id} sx={{ marginBottom: 4, boxShadow: 3 }}>
          <CardHeader title={`Đăng vào: ${new Date(photo.date_time).toLocaleString()}`} />
          <CardMedia component="img" image={`/images/${photo.file_name}`} sx={{ width: '100%' }} />
          <CardContent>
            <Typography variant="h6" gutterBottom>Bình luận:</Typography>
            <Divider />
            <List>
              {photo.comments && photo.comments.map((c) => (
                <ListItem key={c._id} sx={{ display: 'block', paddingLeft: 0 }}>
                  ...
                  <Typography variant="body2" sx={{ marginTop: 1 }}>{c.comment}</Typography>
                  <Divider variant="inset" component="li" sx={{ marginTop: 1 }} />
                </ListItem>
              ))}
            </List>
          </CardContent>
